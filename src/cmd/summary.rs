...
pub fn run(context: &Context, all: bool, depth: Option<usize>, out: &mut impl Write) -> Result<Output, String> {
    ...
    let text = match context.format {
        Format::Json => {
            serde_json::to_string_pretty(&query::summary_json(context.analysis)).map_err(|e| e.to_string())?
        }
